...
import me.farshad.dsl.builder.OperationBuilder
...
class OperationBuilderTest {
    ...
    fun testOperationWithParameters() {
        ...
        operationBuilder.parameter("includeDeleted", ParameterLocation.QUERY, PropertyType.BOOLEAN, false, "Include deleted users")
    ...
    fun testOperationWithParameterFormats() {
        val operationBuilder = OperationBuilder()
        operationBuilder.parameter("date", ParameterLocation.QUERY, PropertyType.STRING, false, "Date parameter", SchemaFormat.DATE_TIME)
        operationBuilder.parameter("timestamp", ParameterLocation.QUERY, PropertyType.STRING, false, "Timestamp", SchemaFormat.DATE_TIME)
        operationBuilder.parameter("userId", ParameterLocation.QUERY, PropertyType.INTEGER, false, "User ID", SchemaFormat.INT64)
        ...
        assertEquals(SchemaFormat.DATE_TIME, operation.parameters?.find { it.name == "date" }?.schema?.format)
        assertEquals(SchemaFormat.DATE_TIME, operation.parameters?.find { it.name == "timestamp" }?.schema?.format)
        assertEquals(SchemaFormat.INT64, operation.parameters?.find { it.name == "userId" }?.schema?.format)
    }
