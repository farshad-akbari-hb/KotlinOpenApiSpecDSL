...
class JsonElementYamlSerializerTest {
    ...
    fun testSerializeJsonArray() {
        val simpleArray = buildJsonArray {
            add("string")
            add(123)
            add(true)
            add(JsonNull)
    ...
    fun testSerializeJsonObject() {
        val simpleObject = buildJsonObject {
            put("name", "John Doe")
            put("age", 30)
            put("active", true)
            put("salary", JsonNull)
    ...
    fun testSerializeArrayOfObjects() {
        val arrayOfObjects = buildJsonArray {
            add(buildJsonObject {
                put("id", 1)
                put("name", "Item 1")
            })
        ...
        val itemCount = yamlString.count { it == '-' && yamlString.indexOf(it) == 0 || yamlString.substring(yamlString.indexOf(it) - 1, yamlString.indexOf(it)) == "\n" }
        assertTrue(itemCount >= 3)
    }
    
    @Test
    fun testSerializeObjectWithArrayValues() {
        val objectWithArrays = buildJsonObject {
            put("tags", buildJsonArray {
                add("kotlin")
                add("serialization")
                add("yaml")
            })
            put("numbers", buildJsonArray {
                add(1)
                add(2)
                add(3)
                add(4)
    ...
    fun testSerializeSpecialYamlValues() {
        ...
        val specialValues = buildJsonObject {
    ...
    fun testSerializeUnicodeAndSpecialCharacters() {
        val unicodeObject = buildJsonObject {
            put("emoji", "🎉🎊✨")
            put("chinese", "你好世界")
